...
pub fn load_dbc(path: &str, state: &DiagnosticsState) -> Result<String, String> {
...
pub fn clear_dbc(state: &DiagnosticsState) -> Result<(), String> {
    state.clear_dbc();
...
pub fn get_dbc_path(state: &DiagnosticsState) -> Option<String> {
    state.dbc_path.lock().clone()
}

/// Decode a single CAN frame.
pub fn decode_single_frame(frame: &CanFrameDto, state: &DiagnosticsState) -> DecodeResponse {
...
pub fn decode_frames(frames: &[CanFrameDto], state: &DiagnosticsState) -> DecodeResponse {
    let dbc_guard = state.dbc.lock();
    let Some(ref dbc) = *dbc_guard else {
        return DecodeResponse {
            signals: Vec::new(),
...
pub fn save_dbc_content(path: &str, content: &str, state: &DiagnosticsState) -> Result<(), String> {
...
pub fn save_dbc_info(path: &str, info: &DbcInfo, state: &DiagnosticsState) -> Result<(), String> {
    let content = export_dbc_info_to_string(info);
    save_dbc_content(path, &content, state)
}

/// Update in-memory DBC from content string.
pub fn update_dbc_content(content: &str, state: &DiagnosticsState) -> Result<String, String> {
    let dbc = Dbc::parse(content).map_err(|e| format!("Failed to parse DBC: {e:?}"))?;
    let msg_count = dbc.messages().len();
...
pub fn update_dbc_info(info: &DbcInfo, state: &DiagnosticsState) -> Result<String, String> {
    let content = export_dbc_info_to_string(info);
    update_dbc_content(&content, state)
}

/// Get information about the loaded DBC.
pub fn get_dbc_info(state: &DiagnosticsState) -> Result<Option<DbcInfo>, String> {
    let dbc_guard = state.dbc.lock();
    let Some(ref dbc) = *dbc_guard else {
        return Ok(None);
    };
